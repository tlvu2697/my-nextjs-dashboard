import { query as Query } from '@/app/lib/db';
import {
  CustomerField,
  CustomersTableType,
  InvoiceForm,
...
function getRandomArbitrary(min: number = 100, max: number = 3000) {
  return Math.random() * (max - min) + min;
}

async function delay(ms: number = getRandomArbitrary()): Promise<void> {
...
export async function fetchInvoicesPages(query: string) {
  try {
    const count = await Query(`SELECT COUNT(*)
    FROM invoices
    JOIN customers ON invoices.customer_id = customers.id
    WHERE
      customers.name ILIKE ${`%${query}%`} OR
